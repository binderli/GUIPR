...
public class ColorEditor extends Dialog
{
	...
	public void createContent(Shell shell)
	{
		...
		shell.setText(message.getString("colorEditor"));

		// Label Text
		textLabel = new Label(shell, SWT.LEFT);
		textLabel.setText(message.getString("colorTextLabel"));
		...
		gSpinner.setText(message.getString("colorSpinner"));
		...
		colorNames[0].setText(message.getString("red"));
		colorNames[1].setText(message.getString("green"));
		colorNames[2].setText(message.getString("blue"));

		// Buttons
		GridData buttonData = new GridData(SWT.FILL, SWT.FILL, true, true, 1, 1);
		buttonOK = new Button(shell, SWT.PUSH);
		buttonOK.setText(message.getString("colorButtonOk"));
		buttonOK.setLayoutData(buttonData);

		buttonCancel = new Button(shell, SWT.PUSH);
		buttonCancel.setText(message.getString("colorButtonCancel"));
		...
		for (int i = 0; i < spinBoxRGB.length; i++)
		{
			spinBoxRGB[i].addModifyListener(new ModifyListener()
			{
				...
				public void modifyText(ModifyEvent arg0)
				{
					...
					Color oldColor = colorLabel.getBackground();
					oldColor.dispose();
